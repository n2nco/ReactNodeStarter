...
const InputForm = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      let res = await createPaymentIntent(values, store.state) 

      console.log('response from  createPaymentIntent from inputForm onsumbit:')
      // alert(res)
      console.log ('dispatch to store to set client secret:' + res?.data?.clientSecret)
      store.dispatch({type: 'setClientSecret', message: res?.data?.clientSecret})
      store.dispatch({type: 'setPaymentIntent', message: res?.data?.paymentIntent})
  ...
  const onDivWrapperChange = (e) => {
    ...
    if (store.state.submitAlreadyPressed === true && editAfterSubmitShowing === false) {
      ...
      console.log(e.target.value);
        toast.info(`Details modified during payment. Please start again.`, {
          position:  "top-center",
          autoClose: 5000,
